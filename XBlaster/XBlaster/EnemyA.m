...
- (id)initWithPosition:(CGPoint)position
{
    if (self = [super initWithPosition:position]) {
        ...
        // Setup the label that will show the score when this entity is killed
...
- (void)collidedWith:(SKPhysicsBody *)body contact:(SKPhysicsContact*)contact
{
    ...
    if (self.health <= 0) {
        ...
        // Drop a score label into the scene to show the player what they scored for that kill
        _scoreLabel.position = self.position;
        
        // Just to be safe we check that the label is not already in the scene. This could happen if the enemy is killed very quickly
        // after spawning. If the label is in the scene then we remove it.
        if (_scoreLabel.parent)
            [_scoreLabel removeFromParent];
        
        [mainScene.hudLayerNode addChild:_scoreLabel];
        
        // Run the label action which will fade it it and out while moving up the screen
