...
@implementation MyScene
{
    ...
    int _currentLevel; //keeps track of level we're on
...
-(void)initializeScene
{
    ...
    self.physicsWorld.contactDelegate = self; //to use didBeginContact and didEndContact methods
...
{
    CGFloat angle = CGPointToAngle(CGPointSubtract(_hookBaseNode.position, _hookNode.position)); //depending on its orientation to the x-axis.
...
-(void)addCatBed
{
    ...
    _bedNode.physicsBody.dynamic = NO; //will ignore all forces and impulses
...
-(void)addSpringsFromArray:(NSArray*)springs
{
    for (NSDictionary *spring in springs)
    {
        ...
        springSprite.physicsBody.collisionBitMask = CNPhysicsCategoryCat | CNPhysicsCategoryBlock | CNPhysicsCategoryEdge;
...
-(void)setupLevel:(int)levelNum
{
    ...
    [self addCatAtPosition:CGPointFromString(level[@"catPosition"])]; //Use with NSStringFromCGPoint (making it)
...
-(SKSpriteNode *)addBlockWithRect:(CGRect)blockRect
{
    ...
    blockSprite.position = blockRect.origin; //CGPoint object. (with x and y values)
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
     ...
     ^(SKPhysicsBody *body, BOOL *stop)
    {
        ...
        {
            [self.delegate requestImagePicker]; //handled by MyScene's delegate, the view controller
            *stop = YES;
            return;
        }
...
//    }
    ...
    if (collision == (CNPhysicsCategoryCat|CNPhysicsCategoryHook))
    {
        //1
        _catNode.physicsBody.velocity = CGVectorMake(0, 0); //i.e. make it stop moving immediately.
...
-(void)win
{
    ...
    //3
    [_catNode runAction:[SKAction group:@[[SKAction moveTo:curlPoint duration:0.66],
    ...
    [_catNode runAction:[SKAction animateWithTextures:@[[SKTexture textureWithImageNamed:@"cat_curlup1"],
                                         ...
                                         timePerFrame:0.25]]; //amount of time (in seconds) each texture is displayed
